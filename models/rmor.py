'''Random Mixture of Residuals (rMoR)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, round_g=False):
        ...
        #self.linearg_list = list()
        #for i in range(sum(num_blocks)):
        #    self.linearg_list.append(nn.Linear(in_features=128, out_features=2, bias=True))
        self.linearg_list = nn.ModuleList([nn.Linear(in_features=128, out_features=2, bias=True) 
                                           for i in range(sum(num_blocks))])            
    ...
    def return_activity(self):
        used_blocks = torch.sum(torch.round(torch.stack(self.gate_output_list, dim=1)), dim=1)
        return used_blocks
